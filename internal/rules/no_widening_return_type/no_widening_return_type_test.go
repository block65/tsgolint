...
// "tsconfig.json" content under "files" (e.g. an ambient-global case that
// needs its own compiler options) is compiled against that file instead of
// the package's default tsconfig.minimal.json.
	...
	for _, test := range cases {
		t.Run(test.Name, func(t *testing.T) {
			...
			if len(test.Expect) == 0 {
				rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.minimal.json", t, &NoWideningReturnTypeRule,
				...
				for _, expected := range test.Expect {
					...
					errors = append(errors, rule_tester.InvalidTestCaseError{MessageId: "widenedReturn", Line: number})
				}
				rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.minimal.json", t, &NoWideningReturnTypeRule, nil,
				...
				assertPluginCaseMessages(t, &NoWideningReturnTypeRule, code, test.Filename, test.Options, tsconfig, test.Files, test.Messages, test.NotMessages)
			}
		})
	}
}
