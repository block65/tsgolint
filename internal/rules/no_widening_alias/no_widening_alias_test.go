	...
	for _, test := range cases {
		t.Run(test.Name, func(t *testing.T) {
			...
			if len(test.Expect) == 0 {
				rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.minimal.json", t, &NoWideningAliasRule,
				...
				for _, expected := range test.Expect {
					...
					errors = append(errors, rule_tester.InvalidTestCaseError{MessageId: "widenedAlias", Line: number})
				}
				rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.minimal.json", t, &NoWideningAliasRule, nil,
				...
				assertPluginCaseMessages(t, &NoWideningAliasRule, code, test.Filename, test.Options, tsconfig, test.Files, test.Messages, test.NotMessages)
			}
		})
	}
}
