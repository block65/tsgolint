	...
	for _, test := range cases {
		t.Run(test.Name, func(t *testing.T) {
			...
			if len(test.Expect) == 0 {
				rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.minimal.json", t, &DefineMessagesKeysRule,
				...
				for _, expected := range test.Expect {
					...
					errors = append(errors, rule_tester.InvalidTestCaseError{MessageId: "finiteKeys", Line: number})
				}
				rule_tester.RunRuleTester(fixtures.GetRootDir(), "tsconfig.minimal.json", t, &DefineMessagesKeysRule, nil,
				...
				assertPluginCaseMessages(t, &DefineMessagesKeysRule, code, test.Filename, test.Options, tsconfig, test.Files, test.Messages, test.NotMessages)
			}
		})
	}
}
